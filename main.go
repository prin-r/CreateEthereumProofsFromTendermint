...
func handleStore(w http.ResponseWriter, r *http.Request) {
	...
	txHash, err := tx.SendTransaction(zoracle.NewMsgStoreCode(code, tx.Sender()))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	m := map[string]string{}
	m["txHash"] = txResponse.Hash
	m["height"] = txResponse.Height
	m["events"] = fmt.Sprintf("%v", txResponse.TxResult.Events)
...
func main() {
	...
	r.HandleFunc("/store", handleStore).Methods("POST")
