...
type BlockHeaderMerkleParts struct {
	VersionAndChainIdHash       string `json:"height"`
	TimeHash                    string `json:"height"`
	TxCountAndLastBlockInfoHash string `json:"height"`
	ConsensusDataHash           string `json:"height"`
	LastResultsHash             string `json:"height"`
	EvidenceAndProposerHash     string `json:"height"`
}
...
type Signatures struct {
	...
	S                string `json:"s"`
	V                int    `json:"v"`
	SignedDataSuffix string `json:"signeddatasuffix"`
}

type BlockProof struct {
	Height                uint64          `json:"height"`
	OracleIAVLStateHash   string          `json:"oracleiavlstatehash"`
	OtherStoresMerkleHash string          `json:"otherstoresmerklehash"`
	MerkleParts           []string        `json:"merkleparts"`
	SignedDataPrefix      string          `json:"signeddataprefix"`
...
func GetBlockProof(blockId uint64, pk string) (BlockProof, error) {
	...
	bhmp.VersionAndChainIdHash = fmt.Sprintf("0x%x", others[0])
	bhmp.TimeHash = fmt.Sprintf("0x%x", others[1])
	bhmp.TxCountAndLastBlockInfoHash = fmt.Sprintf("0x%x", others[2])
	bhmp.ConsensusDataHash = fmt.Sprintf("0x%x", others[3])
	bhmp.LastResultsHash = fmt.Sprintf("0x%x", others[4])
	bhmp.EvidenceAndProposerHash = fmt.Sprintf("0x%x", others[5])

	bp.MerkleParts = []string{
		bhmp.VersionAndChainIdHash,
		bhmp.TimeHash,
		bhmp.TxCountAndLastBlockInfoHash,
		bhmp.ConsensusDataHash,
		bhmp.LastResultsHash,
		bhmp.EvidenceAndProposerHash,
	}

	leftMsgCount := map[string]int{}
	bp.Signatures = []([]interface{}){}
	validPrecommitCount := 0

	for i := 0; i < len(commit.Precommits); i++ {
		if commit.Precommits[i] == nil {
			continue
		}
		validPrecommitCount += 1
		...
		sig := fmt.Sprintf("%x", vote.Signature)

		bp.Signatures = append(bp.Signatures, []interface{}{
			"0x" + sig[:64],
			"0x" + sig[64:],
			27,
			"0x" + lr[1],
	...
	if validPrecommitCount < 4 {
		return BlockProof{}, fmt.Errorf("Too many invalid precommits")
	}

	maxCount := 0
	for k, count := range leftMsgCount {
		if count > maxCount {
			bp.SignedDataPrefix = "0x" + k
			maxCount = count
		}
	}
